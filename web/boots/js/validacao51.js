...
function validardadosPais()
{
    ...
    if (nome.length == 0)
    {
        ...
        document.dados.nome.style.backgroundColor = "white";
    ...
    if (siglaPais.length != 3)
    {
        ...
        document.dados.siglaPais.style.backgroundColor = "white";
...
function validardadosEstado()
{
    ...
    if (siglaEstado.length != 2)
    {
        ...
        document.dados.siglaEstado.style.backgroundColor = "white";
...
function validardadosCidade()
{
    ...
    // Recupera o valor do combobox
    paisOpcao = document.getElementById('comboPais').options[document.getElementById('comboPais').selectedIndex].value;
    // Verifica se selecionou uma opcao
    if (paisOpcao == "0")
    {
        erros = erros + " País\n";
...
function validardadosMTD( event )
{
    erros = "Campos incorretos:\n";
    id = document.dados.id.value;
    nome = document.dados.nome.value;
    ...
    $.ajax({
        type: "GET",
        url: "/CompetitionsWEB/controlador?parametro=verificarNome&nome=" + nome + "&id=" + id,
        async: false

    }).done(function (retorno) {
        console.log(retorno);
        if (retorno == 'false') {
            erros = erros + " Este Nome já existe\n";
        }
